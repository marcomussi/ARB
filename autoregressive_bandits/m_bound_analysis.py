...
if __name__ == '__main__':
    ...
    out_folder = 'autoregressive_bandits/output/m_bound_analysis/'
    try:
        os.mkdir(out_folder)
    ...
    f = open(f'autoregressive_bandits/input/testcase_m_bound_analysis.json')
    ...
    core = Core(env, agent)
    clairvoyant_logs = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)[:, len(param_dict['X0']):]
    ...
    for m in m_values:
        ...
        core = Core(env, agent)
        arb_logs[m] = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)[
            :, len(param_dict['X0']):]
        regret[arb[m]] = np.inf * np.ones((param_dict['n_epochs'], T))
        for i in range(param_dict['n_epochs']):
            regret[arb[m]][i, :] = clairvoyant_logs[i, :] - arb_logs[m][i, :]
    ...
    plt.savefig(out_folder+f"png/testcase_m_bound_analysis.png")
    tikz.save(out_folder+f"tex/testcase_m_bound_analysis.tex")
    # plt.show()
